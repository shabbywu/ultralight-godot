...
public:
	...
	///
	ultralight::RefPtr<ultralight::View> createView(uint32_t width, uint32_t height, const ViewConfig &config,
			RefPtr<Session> session) {
		return render->CreateView(width, height, config, session);
	}
};
} //namespace gdbind
