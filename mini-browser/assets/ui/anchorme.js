...
var dictionary = createCommonjsModule(function(module, exports) {
    ...
    exports.parenthesis = exports.openingParenthesis.split("").map(function(x, i) {
        return "" + x + exports.closingParenthesis.charAt(i);
    });
    exports.htmlAttributes = [
        "src", "data", "href", "cite", "formaction", "icon", "manifest", "poster", "codebase", "background", "profile",
        "usemap", "itemtype", "action", "longdesc", "classid", "archive"
    ];
...
var regex = createCommonjsModule(function(module, exports) {
    ...
    var domainWithAnyTLD = "([a-z0-9]+(-+[a-z0-9]+)*\\.)+([a-z0-9][a-z0-9-]{0," +
                           (Math.max.apply(commonjsGlobal, dictionary.TLDs.split("|").map(function(x) {
                               return x.length;
                           })) -
                            2) +
                           "}[a-z0-9])";
    var allowedInPath = "a-zA-Z\\d\\-._~\\!$&*+,;=:@%'\"\\[\\]()";
    var path = "(((\\/(([" + allowedInPath + "]+(\\/[" + allowedInPath + "]*)*))?)?)((\\?([" + allowedInPath +
               "\\/?]*))?)((\\#([" + allowedInPath + "\\/?]*))?))?";
    ...
    var fqdn = "(((" + protocol + ")?(" + domainWithTLD + "|" + ipv4 + "|(" + protocol + ")(" + ipv6 + "|" +
               domainWithAnyTLD + "))(?!@\\w)" + port + ")|(" + confirmedByProtocol + "))";
    var nonLatinMatches = fqdn + "((((\\/(([" + allowedInPath + "]+(\\/[" + allowedInPath +
                          dictionary.nonLatinAlphabetRanges + "]*)*))?)?)((\\?([" + allowedInPath +
                          "\\/?]*))?)((\\#([" + allowedInPath + "\\/?]*))?))?\\b((([" + allowedInPath + "\\/" +
                          dictionary.nonLatinAlphabetRanges + "][a-zA-Z\\d\\-_~+=\\/" +
    ...
    var testers = [
        "file:///some/file/path/filename.pdf", "mailto:e+_mail.me@sub.domain.com",
        "http://sub.domain.co.uk:3000/p/a/t/h_(asd)/h?q=abc123#dfdf", "http://www.\u0639\u0631\u0628\u064A.com",
        "http://127.0.0.1:3000/p/a/t_(asd)/h?q=abc123#dfdf", "http://[2a00:1450:4025:401::67]/k/something",
        "a.org/abc/\u10D8_\u10D2\u10D2"
    ].join(" ");
    var result = null;
    var i = 0;
...
var utils = createCommonjsModule(function(module, exports) {
    ...
    exports.maximumAttrLength = dictionary.htmlAttributes
                                    .sort(function(a, b) {
    ...
    function isInsideAnchorTag(target, fullInput, targetEnd) {
        ...
        var regex = new RegExp(
            "(?=(<a))(?!([\\s\\S]*)(<\\/a>)(" + escapedTarget + "))[\\s\\S]*?(" + escapedTarget + ")(?!\"|')", "gi");
        var result = null;
...
var node = createCommonjsModule(function(module, exports) {
    ...
    var list = function(input) {
        ...
        var _loop_1 = function() {
            ...
            if (input.substring(end, input.length).indexOf("</a>") > -1 &&
                input.substring(0, start).indexOf("<a") > -1 && utils.isInsideAnchorTag(string, input, end)) {
                return "continue";
            }
            if (result[regex.iidxes.isURL]) {
                var path = (result[regex.iidxes.url.path] || "") + (result[regex.iidxes.url.secondPartOfPath] || "") ||
                           undefined;
                var protocol = result[regex.iidxes.url.protocol1] || result[regex.iidxes.url.protocol2] ||
                ...
                found.push({
                    ...
                    ipv4 : result[regex.iidxes.url.ipv4Confirmation] ? result[regex.iidxes.url.ipv4] : undefined,
    ...
    var anchorme = function(arg) {
        var _a = typeof arg === "string" ? {input : arg, options : undefined, extensions : undefined} : arg,
            input = _a.input, options = _a.options, extensions = _a.extensions;
        if (extensions) {
        ...
        for (var index = 0; index < found.length; index++) {
            newStr = (newStr        ? newStr
                      : index === 0 ? input.substring(0, found[index].start)
                                    : "") +
                     transform_1.transform(found[index], options) +
                     (found[index + 1] ? input.substring(found[index].end, found[index + 1].start)
    ...
    anchorme.validate = {
        ip : function(input) {
            return regex.ipRegex.test(input);
        },
        email : function(input) {
            return regex.emailRegex.test(input);
        },
        file : function(input) {
            return regex.fileRegex.test(input);
        },
        url : function(input) {
            return regex.urlRegex.test(input) || regex.ipRegex.test(input);
        },
    };
